function __ph_structure_large_blue_mushroom(px, py, seed)
{
	
	
	// worldgen_set_seed(px, py, seed);
	
	var mushroom = ITEM.BLUE_MUSHROOM_BLOCK;
	...
	for (var j = 0; j < 4; ++j)
	{
		for (var k = -2; k < 3; ++k)
		{
			if (j == 3) && (k == -2 || k == 2)
			{
				if (irandom(2) > 0)
				{
					tile_place(px + k, py, CHUNK_DEPTH_DEFAULT + choose(-1, 1), new Tile(ITEM.BLUE_MUSHROOM));
				}
			...
			{
				tile_place(px + k, py - 1, CHUNK_DEPTH_DEFAULT + choose(-1, 1), new Tile(ITEM.BLUE_MUSHROOM));
			}
		}
	...
	for (var o = -4; o < 9; ++o)
	{
		...
		{
			tile_place(px + o, py, CHUNK_DEPTH_DEFAULT - 1, new Tile(ITEM.BLUE_MUSHROOM));
